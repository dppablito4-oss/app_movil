...
async function openTokenView(tokenCode) {
  ...
  if (state.isDemoMode) {
    ...
    const supabase = getSupabase();
    try {
      // Intento 1: Búsqueda directa por tabla con relación a trabajo e items
      const { data: directToken } = await supabase
        .from('qr_tokens')
        .select('id, token, status, job_id, jobs(*, job_items(*))')
        .eq('token', tokenCode)
        .maybeSingle();

      if (directToken) {
        tokenRecord = directToken;
        if (directToken.jobs) {
          jobRecord = directToken.jobs;
          jobRecord.items = directToken.jobs.job_items || [];
        }
      } else {
        // Intento 2: RPC pública get_public_job_by_token
        ...
        if (res) {
          if (res.status === 'unused') {
            ...
            tokenRecord = { token: tokenCode, status: 'assigned', job_id: res.job.id || tokenCode };
            jobRecord = {
              id: res.job.id || tokenCode,
              ...
              total_cents: res.job.total_cents,
      ...
      console.warn('Error fetching token/job info:', err);
    }
  }
...
function renderAdminJobDetailCard(container, job, tokenCode) {
  ...
  container.querySelectorAll('.btn-status-change').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      btn.disabled = true;
      await updateJobStatus(job.id, newStatus, tokenCode);
      await openTokenView(tokenCode);
    });
  });
...
async function updateJobStatus(jobId, newStatus, tokenCode = null) {
  const timestamps = {};
  ...
  if (state.isDemoMode) {
    ...
    if (!targetJobId || targetJobId === 'remote-job') {
      if (tokenCode) {
        const { data: tok } = await supabase.from('qr_tokens').select('job_id').eq('token', tokenCode).maybeSingle();
        if (tok && tok.job_id) targetJobId = tok.job_id;
      }
    }
    if (targetJobId && targetJobId !== 'remote-job') {
      await supabase.from('jobs').update({ status: newStatus, ...timestamps, updated_at: new Date().toISOString() }).eq('id', targetJobId);
    }
  }
  await fetchJobs();
}
...
function buildServiceSelectOptions(selectedLabel) {
  const categories = {};
  ...
  for (const [catName, list] of Object.entries(categories)) {
    ...
    list.forEach(s => {
      const isSel = selectedLabel && (selectedLabel === s.name || selectedLabel.toLowerCase().trim() === s.name.toLowerCase().trim());
      if (isSel) isKnownPreset = true;
      html += `<option value="${escapeHtml(s.name)}" ${isSel ? 'selected' : ''}>${escapeHtml(s.name)}</option>`;
    });
    html += `</optgroup>`;
  }

  const isCustom = selectedLabel && !isKnownPreset && selectedLabel.trim() !== '';
  html += `
    <optgroup label="✍️ Personalizado">
      <option value="__custom__" ${isCustom ? 'selected' : ''}>✏️ Escribir otro servicio personalizado...</option>
    </optgroup>
  `;
...
function addItemRow(label = '', qty = 1, price = null, isFolleto = false) {
  ...
  const defaultLabel = label || (PRESET_SERVICES[0] ? PRESET_SERVICES[0].name : '');
  const matched = findPresetByLabel(defaultLabel);
  ...
  const { html: serviceOptionsHtml, isCustom } = buildServiceSelectOptions(defaultLabel);
  ...
  rowCard.innerHTML = `
    <div class="item-row-main">
      <div class="item-col-label">
        ...
        <select class="form-control item-service-select" title="Selecciona el servicio">
          ${serviceOptionsHtml}
        </select>
        <input type="text" class="form-control item-custom-label ${isCustom ? '' : 'hidden'}" placeholder="Escribe el nombre del servicio..." value="${isCustom ? escapeHtml(label) : ''}">
      </div>
  ...
  const serviceSelect = rowCard.querySelector('.item-service-select');
  const customLabelInput = rowCard.querySelector('.item-custom-label');
  ...
  const updateChipsAndSelect = (preset) => {
    ...
    priceSelect.innerHTML = '<option value="">Sugerir...</option>' + list.map(p => `
  ...
  serviceSelect.addEventListener('change', () => {
    ...
    if (val === '__custom__') {
      ...
      updateChipsAndSelect(null);
    } else {
      customLabelInput.classList.add('hidden');
      const p = findPresetByLabel(val);
      if (p) {
        priceInput.value = p.defaultPrice.toFixed(2);
        folletoCheck.checked = !!p.isFolleto;
  ...
  customLabelInput.addEventListener('input', calculateTotal);
...
async function handleJobFormSubmit(e) {
  ...
  const items = [];
  document.querySelectorAll('#job-items-container .item-row-card, #job-items-container .item-row').forEach(row => {
    const selectVal = row.querySelector('.item-service-select')?.value;
    const customVal = row.querySelector('.item-custom-label')?.value.trim();
    const legacyLabel = row.querySelector('.item-label')?.value?.trim();

    let label = legacyLabel || '';
    if (selectVal) {
      label = (selectVal === '__custom__' ? customVal : selectVal) || customVal || 'Servicio';
    }
